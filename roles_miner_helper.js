...
var helper = {
	...
	action: function () {
		...
		if (!target) {
			...
			var emptySource = creep.pos.findNearest(Game.MY_SPAWNS, {
				ignoreCreeps: true,
				filter: function (spawn) {
					return spawn.energy / spawn.energyCapacity <= .25;
				}
			});
			...
			var builderToHelp = creep.pos.findNearest(Game.MY_CREEPS, {
				filter: function (builder) {
					return builder.memory.role == "builder"
						&& builder.energy < builder.energyCapacity;
				}
			});
		...
		if (!target) {
			var notFullSpawn = creep.pos.findNearest(Game.MY_SPAWNS, {
				ignoreCreeps: true,
				filter: function (spawn) {
		...
		if (!target) {
			var notFullExtension = creep.pos.findNearest(Game.MY_STRUCTURES, {
				filter: function (structure) {
					return (structure.structureType == Game.STRUCTURE_EXTENSION
